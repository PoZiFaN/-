...
class PoseDetector:
    ...
    def find_pose(self, frame) -> dict | None:
        ...
        try:
            ...
            # Порог снижен с 0.5 до 0.30, чтобы система стабильно удерживала захват человека
            # при небольших изменениях освещения, наличии свободной одежды или мелких теней.
            if any(kp[i][2] < 0.30 for i in [si, hi, ki, ai]):
                self.smoother.reset()
                return None
